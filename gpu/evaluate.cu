...
{
    if (argc < 3) {
        fprintf(stderr, "Usage: gpu_enumerate <task.json> <kernel_depth> [max_solutions]\n");
        return 1;
    }
    ...
    if (depth < 0 || depth > MAX_KERNEL_DEPTH) {
        fprintf(stderr, "kernel_depth must be in [0, %d]\n", MAX_KERNEL_DEPTH);
        return 1;
    }
    ...
    if (spec.num_inputs + depth + 2 > MAX_PROG_LEN) {
    ...
    printf("  candidates/position: %d  kernel_depth: %d  total programs: %" PRIu64 "\n",
           nc, depth, total_candidates);
    ...
    if (stored > 0) {
        ...
        for (int i = 0; i < stored; i++) {
            ...
            solutions[i].passed_tests = task_evaluate(&spec, solutions[i].words, solutions[i].len);
        }
